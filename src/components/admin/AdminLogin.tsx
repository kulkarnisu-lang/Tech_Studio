...
import { Terminal, Shield, Lock, ArrowLeft, KeyRound, AlertCircle, CheckCircle2 } from 'lucide-react';
import { useAdmin } from '../../context/AdminContext';

export default function AdminLogin() {
  const { login, closeAdmin, companyConfig } = useAdmin();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (!success) {
        setError('Invalid passcode. Use demo credential: admin123');
        ...
        <div className="bg-[#0C111A] border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="p-3.5 mb-6 rounded-lg bg-slate-900/90 border border-slate-800/80 text-xs text-slate-300 space-y-1">
            ...
            <p className="text-slate-400 text-[11px] leading-relaxed">
              Manage incoming quality consultation leads, client inquiries, website configuration, service offerings, and leadership profiles.
            </p>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-slate-800/80">
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-slate-400 font-mono text-[11px]">Instant Demo Credential:</span>
              <code className="px-2 py-0.5 rounded bg-slate-800 text-sky-300 font-mono text-xs font-semibold">
                admin123
              </code>
            </div>
            <button
